...
class NeuralNetwork():
  ...
  def _build_model(self):
    ...
    # Build the network
    self.s = tf.placeholder(tf.float32, [None, N_STATES], name="state")
    #layer0 = tf.contrib.layers.fully_connected(None, N_STATES, tf.nn.relu)
    layer_1 = tf.contrib.layers.fully_connected(self.s, N_NODES, tf.nn.relu)
    layer_2 = tf.contrib.layers.fully_connected(layer_1, N_NODES, tf.nn.relu)
    self.layer_3_output = tf.contrib.layers.fully_connected(layer_2, N_ACTIONS, activation_fn=None)


    self.target = tf.placeholder(tf.float32)

    self.calculate_loss = tf.reduce_sum(tf.losses.mean_squared_error(self.target, np.max(self.layer_3_output)))

    optimizer = tf.train.AdamOptimizer(0.01)
    self.train = optimizer.minimize(self.calculate_loss)
  ...
  def predict(self, sess, states):
    ...
    prediction = sess.run(self.layer_3_output, feed_dict)
  ...
  def update(self, sess, states, actions, targets):
    ...
    """
    # TODO: Implement this!
    #sess.run(self.train, {self.s:[states], self.target:[targets]})
    loss = sess.run(self.calculate_loss, {self.s:[states], self.target:[actions]})
